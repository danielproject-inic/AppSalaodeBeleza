                    ...
                    <div className="absolute top-full left-4 mt-2 z-[110] bg-[#0f172a] border border-white/10 rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.8)] p-5 w-80 animate-in fade-in zoom-in-95 duration-200 backdrop-blur-xl">
                                ...
                                return (
                                    <div key={day} className={`relative flex items-center justify-center h-8 w-full ${isInRange ? 'bg-[#b45309]/20' : ''} ${isStart && tempEnd ? 'bg-gradient-to-r from-transparent to-[#b45309]/20' : ''} ${isEnd && tempStart ? 'bg-gradient-to-r from-[#b45309]/20 to-transparent' : ''}`}>
                                        <button
                                            ...
                                            className={`h-8 w-8 rounded-full text-[11px] font-black transition-all flex items-center justify-center relative z-10 ${
                                                isStart || isEnd ? 'bg-[#b45309] text-white shadow-[0_0_15px_rgba(180,83,9,0.6)] scale-110' : isInRange ? 'text-[#b45309] hover:bg-[#b45309]/30' : 'text-white/70 hover:bg-white/10 hover:scale-110'
                                            }`}
                                        >
                                            {day}
                                        </button>
                        ...
                        <div className="mt-6 pt-4 border-t border-white/5 flex justify-between items-center relative z-10">
                            ...
                            <button 
                                ...
                                disabled={!tempStart}
                                className={`px-5 py-2.5 text-[9px] font-black uppercase tracking-[0.2em] rounded-xl transition-all ${
                                    tempStart 
...
const DashboardOverview: React.FC<DashboardOverviewProps> = ({ onNavigate }) => {
    const { profile, role, professionalId, hasAccess } = useCurrentUserRef();
    ...
    const { commissions, batches } = useCommissions(undefined, canViewAll ? undefined : (professionalId || undefined));
    
    const pendingBatchesCount = useMemo(() => {
        if (!professionalId) return 0;
        return batches?.filter(b => b.status === 'pending_collaborator' && b.data?.professionalId === professionalId).length || 0;
    }, [batches, professionalId]);
    ...
    return (
        <div className="w-full py-4 px-2 lg:px-8 selection:bg-[#b45309]/20 relative custom-scrollbar font-sans text-white">
            <main className="max-w-[1180px] mx-auto flex flex-col gap-3 pb-20">
                    ...
                    <div className="flex items-center gap-1">
                        {['Diário', 'Mês', 'Ano', 'Personalizado'].map((period) => (
                            <button
                                ...
                                className={`px-4 h-9 rounded-full text-xs font-black uppercase tracking-wider transition-all flex items-center gap-0.5 ${
                ...
                <div className="bg-[#111827] border border-white/5 rounded-2xl p-1.5 shadow-xl mt-2">
                    <div className="relative flex items-center justify-between h-12 px-4 bg-[#1f2937]/50 rounded-t-xl border-b border-white/5 z-10">
                        <h3 className="relative text-[11px] font-black text-white/40 uppercase tracking-[0.3em]">
                        ...
                        <div className="flex items-center gap-4 ml-auto">
                            {canViewAll && (
                                <div className="flex items-center gap-2">
                                    <span className="text-[9px] text-white/40 uppercase font-bold">Colaborador:</span>
                                    <select 
                                        className="bg-black/40 border border-white/10 text-[10px] text-white outline-none rounded-sm px-2 py-1 uppercase"
                                        value={historyProId}
                            ...
                            <div className="flex items-center gap-2">
                                <span className="text-[9px] text-white/40 uppercase font-bold">Data:</span>
                                <input 
                                    type="date" 
                                    className="bg-black/40 border border-white/10 text-[10px] text-white outline-none rounded-sm px-2 py-1 uppercase"
                                    value={historyDate}
                                ...
                                {(historyProId !== 'all' || historyDate !== '') && (
                                    <button 
                                        className="text-[9px] text-[#b45309] uppercase font-black hover:text-[#d97706] ml-2 tracking-widest transition-colors"
                        ...
                        <table className="w-full text-left" style={{ borderCollapse: 'collapse' }}>
                            <thead className="bg-[#1f2937]/80 sticky top-0 z-10 backdrop-blur-md">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 text-[9px] font-black text-white/40 uppercase tracking-widest border-b border-white/5">Cliente</th>
                                    {canViewAll && <th className="px-4 py-3 text-[9px] font-black text-white/40 uppercase tracking-widest border-b border-white/5">Colaborador</th>}
                                    <th className="px-4 py-3 text-[9px] font-black text-white/40 uppercase tracking-widest border-b border-white/5">Serviço</th>
                                    <th className="px-4 py-3 text-[9px] font-black text-white/40 uppercase tracking-widest border-b border-white/5 text-center">Valor Bruto</th>
                                    <th className="px-4 py-3 text-[9px] font-black text-[#b45309] uppercase tracking-widest border-b border-white/5 text-center">Comissão</th>
                                    <th className="px-4 py-3 text-[9px] font-black text-white/40 uppercase tracking-widest border-b border-white/5 text-center">Status</th>
                                </tr>
                            </thead>

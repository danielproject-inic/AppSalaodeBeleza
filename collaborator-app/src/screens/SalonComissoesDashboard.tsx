...
const SalonComissoesDashboard = () => {
  ...
  const enrichBatch = (b: any) => {
    ...
    const safeDateStr = rawDateStr.includes('T') ? rawDateStr : rawDateStr + 'T12:00:00';
    const periodLabel = b.period || b.data?.periodLabel || new Date(safeDateStr).toLocaleDateString('pt-BR');
                ...
                {pendingBatches.map((lote: any) => (
                  <React.Fragment key={lote.id}>
                    ...
                    >
                      ...
                      {canViewAll && <td className={`px-3 py-3 font-bold text-center ${focusedBatchId === lote.id ? 'text-amber-700' : 'text-amber-600/80'}`}>
                        {lote.data?.professionalName || 'Profissional'} <span className="text-[9px] uppercase tracking-widest opacity-60">
                          ({(() => {
                            const r = (lote.data?.professionalRole || '').toLowerCase();
                            ...
                            {historyPeriodFilter === 'Mensal' && group.quinzenas.map((qz: any) => (
                              <div key={qz.id} className="border border-white/5 rounded-sm overflow-hidden mb-1">
                                <div
                                  ...
                                  onClick={(e: React.MouseEvent) => { e.stopPropagation(); setExpandedHistQuinzenaId(expandedHistQuinzenaId === qz.id ? null : qz.id); }}
                                >
                                    ...
                                    {qz.days.map((day: any) => (
                                      <div key={day.id} className="border border-white/5 rounded-sm overflow-hidden">
                                        <div
                                          ...
                                          onClick={(e: React.MouseEvent) => { e.stopPropagation(); setExpandedHistDayId(expandedHistDayId === day.id ? null : day.id); }}
                                        >
                                          <div className="flex items-center gap-2">

...
import { ModuleKey } from '../hooks/useCurrentUserRef';
...
const TeamManagement: React.FC<TeamManagementProps> = ({ hasAccess, currentProfileId }) => {
    ...
    const collaborators: Collaborator[] = useMemo(() => {
        return dbProfessionals.map(p => {
            const addr = (p.address_json as Record<string, any>) || {};
            return {
                id: p.id,
                nome: p.name,
                cpf: p.cpf || '',
                            ...
                            <div className="p-6 overflow-y-auto custom-scrollbar">
                                ...
                                <div className="flex flex-col items-center mb-8 pt-4">
                                    <div
                                        ...
                                        className="size-32 rounded-3xl border-2 border-dashed border-white/10 hover:border-amber-500 cursor-pointer flex items-center justify-center overflow-hidden group transition-all relative bg-[#0f172a] mb-2"
                                    >
                                        {avatarPreview ? (
                                    ...
                                    <div className="flex items-start gap-4 mb-4">
                                        <div className="relative">
                                            {(() => {
                                                ...
                                                return (
                                                    <div
                                                        className={`size-14 rounded-2xl bg-cover bg-center transition-all ring-offset-2 ring-offset-white ${ringClass}`}
                                                        style={{ backgroundImage: collab.avatar ? `url('${collab.avatar}')` : undefined }}
                                                    >
                                                        {!collab.avatar && (
                                                            <div className="size-full flex items-center justify-center bg-white border border-slate-300 shadow-sm hover:shadow-md transition-all rounded-xl text-slate-400 font-bold text-xl rounded-2xl">
                                ...
                                <div className="px-8 pb-4 -mt-10 flex flex-col items-center border-b border-white/5">
                                    <div className="relative group/avatar">
                                        {(() => {
                                            ...
                                            return (
                                                <div
                                                    className={`size-24 rounded-2xl bg-cover bg-center shadow-2xl ring-offset-4 ring-offset-[#1e293b] ${ringClass}`}
                                                    style={{ backgroundImage: selectedCollaborator.avatar ? `url('${selectedCollaborator.avatar}')` : undefined }}
                                                >
                                                    {!selectedCollaborator.avatar && (
                                                        <div className="size-full flex items-center justify-center bg-[#1e293b] border border-white/10 shadow-sm rounded-3xl text-slate-400 font-bold text-5xl">

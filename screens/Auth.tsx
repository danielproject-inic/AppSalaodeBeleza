...
const Auth: React.FC<AuthProps> = ({ onAuthSuccess }) => {
    ...
    React.useEffect(() => {
        const saved = localStorage.getItem('saved_emails_admin');
        if (saved) {
            try {
    ...
    const handleRemoveEmail = (emailToRemove: string) => {
        ...
        localStorage.setItem('saved_emails_admin', JSON.stringify(updated));
    };
        ...
        try {
            if (isRegistering) {
                ...
                setSuccess({
                    ...
                    message: 'Verifique seu e-mail para confirmar sua conta antes de fazer o login.'
                });
            } else {
                ...
                if (!emailsList.includes(email.trim().toLowerCase())) {
                    ...
                    localStorage.setItem('saved_emails_admin', JSON.stringify(emailsList));
                }
                    ...
                    <div className="relative z-20">
                        ...
                        <div className="text-center mb-10">
                            ...
                            <p className="text-white/40 font-medium tracking-wide text-sm px-4 min-h-[20px]">
                                {isRegistering ? 'Junte-se ao novo padrão de gestão de beleza.' : 'O próximo nível da gestão de beleza.'}
                            </p>
                        </div>
                            ...
                            >
                                {loading ? (
                                    ...
                                    <span className="flex items-center gap-2">
                                        {isRegistering ? 'Começar Jornada' : 'Entrar no Painel'}
                        ...
                        <div className="mt-8 text-center pt-8 border-t border-white/5">
                            <p className="text-white/40 text-xs font-medium">
                                {isRegistering ? 'Já possui uma conta?' : 'Novo Usuário?'}
                                <button
                                    onClick={() => setIsRegistering(!isRegistering)}
                                    className="ml-2 font-black text-white hover:text-[#b45309] transition-colors"
                                >

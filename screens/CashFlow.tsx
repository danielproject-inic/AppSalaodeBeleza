...
const CashFlow: React.FC = () => {
    ...
    const servicesWithPros = useMemo(() => {
        return dbServices.map(s => {
            // Fix: Handle nested structure from Supabase query: service_professionals -> professional
            const specificPros = s.professionals?.map((p: any) => p.professional?.name).filter(Boolean);
    ...
    // Add Service Sub-modal State
    const [tempSelPro, setTempSelPro] = useState('');
    const [tempSelService, setTempSelService] = useState<Service | null>(null);
    ...
    const handleConfirmAddService = () => {
        ...
        const sWithPros = servicesWithPros.find(s => s.id === tempSelService.id);
                                        ...
                                        <div key={session.id} className="p-5 rounded-3xl bg-[#1f2937]/50 border border-white/5 hover:border-white/10 transition-all flex flex-col gap-4 text-left">
                                            ...
                                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 border-t border-white/5 pt-4">
                                                <div>
                                                    ...
                                                    <span className="text-xs font-bold text-white">
                                                        {session.status === 'open' 
                                                            ? (session as any).opened_by_profile?.full_name 
                            ...
                            <div className="p-8 overflow-y-auto custom-scrollbar bg-transparent">
                                ...
                                {modalMode === 'close' && (
                                    <CashClosureModal
                                        ...
                                        actualClosingBalanceInput={actualClosingBalanceInput}
                                        setActualClosingBalanceInput={setActualClosingBalanceInput}
                                        closureObservations={closureObservations}
                                        setClosureObservations={setClosureObservations}

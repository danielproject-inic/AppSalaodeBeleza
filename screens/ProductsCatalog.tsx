...
const ProductsCatalog: React.FC = () => {
 ...
 const status = product.stock_quantity === 0 ? 'Esgotado' :
 product.stock_quantity <= (product.min_stock_level || 5) ? 'Baixo Estoque' : 'Disponível';
 ...
 const status = product.stock_quantity === 0 ? { label: 'Esgotado', color: 'bg-red-500/10 text-red-700 border-red-100', dot: 'bg-red-500/100' } :
 product.stock_quantity <= (product.min_stock_level || 5) ? { label: 'Baixo', color: ' shadow-md text-white/10 text-cyan-700 border-cyan-500/20', dot: ' bg-cyan-500 text-white shadow-md' } :
 ...
 <span className="font-bold text-slate-800">R$ {product.price.toFixed(2)}</span>
 </td>
 <td className="px-4 py-4 text-center">
 <span className={`text-sm font-bold ${product.stock_quantity <= (product.min_stock_level || 5) ? 'text-red-600' : 'text-amber-600'}`}>
 ...
 <span className="text-amber-600">Mostrando 1-4 de 128 produtos</span>
 ...
 selectedProduct.stock_quantity <= (selectedProduct.min_stock_level || 5) ? ' shadow-md text-white/15 text-cyan-700' :
 'bg-green-100 text-green-700'
 }`}>
 {selectedProduct.stock_quantity === 0 ? 'Esgotado' :
 selectedProduct.stock_quantity <= (selectedProduct.min_stock_level || 5) ? 'Baixo' : 'Disponível'}
 </span>
 </div>
 ...
 <p className="text-xl font-black text-[#d4a72c]">R$ {selectedProduct.price.toFixed(2)}</p>
 </div>
 <div className="p-4 rounded-xl bg-white border border-slate-300 shadow-none">
 <p className="text-xs text-amber-600 font-bold uppercase mb-1">Estoque</p>
 <p className="text-xl font-black text-slate-800">{selectedProduct.stock_quantity} <span className="text-xs font-normal text-amber-600">unidades</span></p>
 </div>
 </div>
 ...
 <div className="w-20 text-center font-mono font-bold text-lg">{selectedProduct.stock_quantity}</div>
 <button
 onClick={() => handleUpdateStock(selectedProduct.id, 1)}
 ...
 <button className="w-full py-4 rounded-xl bg-[#0f172a] text-slate-800 font-bold shadow-lg hover:shadow-xl hover:bg-black transition-all flex items-center justify-center gap-2">
